...
TEST(ParseStringTest, ParsesValidModel) {
  ...
  const IntLiteral* const const_init0 = dynamic_cast<const IntLiteral*>(
      model.GetConstantInit(0));
  ...
  const IntLiteral* const const_init1 = dynamic_cast<const IntLiteral*>(
      model.GetConstantInit(1));
  ...
  const DoubleLiteral* const const_init3 = dynamic_cast<const DoubleLiteral*>(
      model.GetConstantInit(3));
  ...
  const IntLiteral* const var_min0 = dynamic_cast<const IntLiteral*>(
      &model.GetVariableRange(0)->min());
  ASSERT_NE(nullptr, var_min0);
  EXPECT_EQ(17, var_min0->value());
  const IntLiteral* const var_max0 = dynamic_cast<const IntLiteral*>(
      &model.GetVariableRange(0)->max());
  ASSERT_NE(nullptr, var_max0);
  EXPECT_EQ(42, var_max0->value());
  const IntLiteral* const var_init0 = dynamic_cast<const IntLiteral*>(
      model.GetVariableInit(0));
  ...
  const BoolLiteral* const var_init1 = dynamic_cast<const BoolLiteral*>(
      model.GetVariableInit(1));
  ...
  const IntLiteral* const var_min2 = dynamic_cast<const IntLiteral*>(
      &model.GetVariableRange(2)->min());
  ASSERT_NE(nullptr, var_min2);
  EXPECT_EQ(0, var_min2->value());
  const IntLiteral* const var_max2 = dynamic_cast<const IntLiteral*>(
      &model.GetVariableRange(2)->max());
  ...
  const IntLiteral* const var_min3 = dynamic_cast<const IntLiteral*>(
      &model.GetVariableRange(3)->min());
  ASSERT_NE(nullptr, var_min3);
  EXPECT_EQ(0, var_min3->value());
  const Identifier* const var_max3 = dynamic_cast<const Identifier*>(
      &model.GetVariableRange(3)->max());
  ASSERT_NE(nullptr, var_max3);
  EXPECT_EQ("c1", var_max3->name());
  const Identifier* const var_init3 = dynamic_cast<const Identifier*>(
      model.GetVariableInit(3));
  ASSERT_NE(nullptr, var_init3);
  EXPECT_EQ("f1", var_init3->name());
  ...
  const IntLiteral* const var_min5 = dynamic_cast<const IntLiteral*>(
      &model.GetVariableRange(5)->min());
  ASSERT_NE(nullptr, var_min5);
  EXPECT_EQ(0, var_min5->value());
  const Identifier* const var_max5 = dynamic_cast<const Identifier*>(
      &model.GetVariableRange(5)->max());
  ASSERT_NE(nullptr, var_max5);
  EXPECT_EQ("c2", var_max5->name());
  const Identifier* const var_init5 = dynamic_cast<const Identifier*>(
      model.GetVariableInit(5));
  ASSERT_NE(nullptr, var_init5);
  EXPECT_EQ("f2", var_init5->name());
  ...
  const IntLiteral* const formula_expr0 = dynamic_cast<const IntLiteral*>(
      &model.formula_expr(0));
  ASSERT_NE(nullptr, formula_expr0);
  EXPECT_EQ(17, formula_expr0->value());
  EXPECT_EQ("f1", model.GetFormulaName(0));
  const IntLiteral* const formula_expr1 = dynamic_cast<const IntLiteral*>(
      &model.formula_expr(1));
  ASSERT_NE(nullptr, formula_expr1);
  EXPECT_EQ(42, formula_expr1->value());
  EXPECT_EQ("f2", model.GetFormulaName(1));
  const BoolLiteral* const formula_expr2 = dynamic_cast<const BoolLiteral*>(
      &model.formula_expr(2));
  ASSERT_NE(nullptr, formula_expr2);
  EXPECT_EQ(true, formula_expr2->value());
  EXPECT_EQ("h", model.GetFormulaName(2));
  const DoubleLiteral* const formula_expr3 = dynamic_cast<const DoubleLiteral*>(
      &model.formula_expr(3));
  ...
  ASSERT_NE(nullptr, model.module_commands(0)[0].action());
  EXPECT_EQ("a1", *model.module_commands(0)[0].action());
  const BinaryOperation* const guard0 = dynamic_cast<const BinaryOperation*>(
      &model.module_commands(0)[0].guard());
  ASSERT_NE(nullptr, guard0);
  EXPECT_EQ(BinaryOperator::LESS, guard0->op());
  const Identifier* const guard0_operand1 = dynamic_cast<const Identifier*>(
      &guard0->operand1());
  ASSERT_NE(nullptr, guard0_operand1);
  EXPECT_EQ("i1", guard0_operand1->name());
  const Identifier* const guard0_operand2 = dynamic_cast<const Identifier*>(
      &guard0->operand2());
  ASSERT_NE(nullptr, guard0_operand2);
  EXPECT_EQ("c1", guard0_operand2->name());
  ASSERT_EQ(1, model.module_commands(0)[0].outcomes().size());
  const BinaryOperation* const probability0 =
  ...
  const IntLiteral* const probability0_operand1 =
      dynamic_cast<const IntLiteral*>(&probability0->operand1());
  ASSERT_NE(nullptr, probability0_operand1);
  EXPECT_EQ(1, probability0_operand1->value());
  const Identifier* const probability0_operand2 =
      dynamic_cast<const Identifier*>(&probability0->operand2());
  ASSERT_NE(nullptr, probability0_operand2);
  EXPECT_EQ("i", probability0_operand2->name());
  ASSERT_EQ(1, model.module_commands(0)[0].outcomes()[0].updates().size());
  EXPECT_EQ("i1",
            model.module_commands(0)[0].outcomes()[0].updates()[0].variable());
  const BinaryOperation* const update_expr0 =
      dynamic_cast<const BinaryOperation*>(
  ...
  const Identifier* const update_expr0_operand1 =
      dynamic_cast<const Identifier*>(&update_expr0->operand1());
  ASSERT_NE(nullptr, update_expr0_operand1);
  EXPECT_EQ("i1", update_expr0_operand1->name());
  const IntLiteral* const update_expr0_operand2 =
      dynamic_cast<const IntLiteral*>(&update_expr0->operand2());
  ...
  EXPECT_EQ("a2", *model.module_commands(1)[0].action());
  const BinaryOperation* const guard1 = dynamic_cast<const BinaryOperation*>(
      &model.module_commands(1)[0].guard());
  ASSERT_NE(nullptr, guard1);
  EXPECT_EQ(BinaryOperator::LESS, guard1->op());
  const Identifier* const guard1_operand1 = dynamic_cast<const Identifier*>(
      &guard1->operand1());
  ASSERT_NE(nullptr, guard1_operand1);
  EXPECT_EQ("i2", guard1_operand1->name());
  const Identifier* const guard1_operand2 = dynamic_cast<const Identifier*>(
      &guard1->operand2());
  ASSERT_NE(nullptr, guard1_operand2);
  EXPECT_EQ("c2", guard1_operand2->name());
  ASSERT_EQ(1, model.module_commands(1)[0].outcomes().size());
  const BinaryOperation* const probability1 =
      dynamic_cast<const BinaryOperation*>(
  ...
  const IntLiteral* const probability1_operand1 =
      dynamic_cast<const IntLiteral*>(&probability1->operand1());
  ASSERT_NE(nullptr, probability1_operand1);
  EXPECT_EQ(1, probability1_operand1->value());
  const Identifier* const probability1_operand2 =
      dynamic_cast<const Identifier*>(&probability1->operand2());
  ASSERT_NE(nullptr, probability1_operand2);
  EXPECT_EQ("i", probability1_operand2->name());
  ASSERT_EQ(1, model.module_commands(1)[0].outcomes()[0].updates().size());
  EXPECT_EQ("i2",
            model.module_commands(1)[0].outcomes()[0].updates()[0].variable());
  const BinaryOperation* const update_expr1 =
      dynamic_cast<const BinaryOperation*>(
  ...
  const Identifier* const update_expr1_operand1 =
      dynamic_cast<const Identifier*>(&update_expr1->operand1());
  ASSERT_NE(nullptr, update_expr1_operand1);
  EXPECT_EQ("i2", update_expr1_operand1->name());
  const IntLiteral* const update_expr1_operand2 =
      dynamic_cast<const IntLiteral*>(&update_expr1->operand2());
  ASSERT_NE(nullptr, update_expr1_operand2);
  EXPECT_EQ(1, update_expr1_operand2->value());

  const Identifier* const label_expr = dynamic_cast<const Identifier*>(
      model.GetLabelExpr("\"l\""));
  ASSERT_NE(nullptr, label_expr);
  EXPECT_EQ("f1", label_expr->name());

  ASSERT_EQ(2, model.num_rewards());
  ASSERT_EQ(1, model.state_rewards(0).size());
  const BinaryOperation* const reward_guard0 =
      dynamic_cast<const BinaryOperation*>(&model.state_rewards(0)[0].guard());
  ASSERT_NE(nullptr, reward_guard0);
  EXPECT_EQ(BinaryOperator::EQUAL, reward_guard0->op());
  const Identifier* const reward_guard0_operand1 =
      dynamic_cast<const Identifier*>(&reward_guard0->operand1());
  ASSERT_NE(nullptr, reward_guard0_operand1);
  EXPECT_EQ("i1", reward_guard0_operand1->name());
  const Identifier* const reward_guard0_operand2 =
      dynamic_cast<const Identifier*>(&reward_guard0->operand2());
  ASSERT_NE(nullptr, reward_guard0_operand2);
  EXPECT_EQ("c1", reward_guard0_operand2->name());
  const DoubleLiteral* const reward_expr0 = dynamic_cast<const DoubleLiteral*>(
      &model.state_rewards(0)[0].reward());
  ...
  const BoolLiteral* const reward_guard1 = dynamic_cast<const BoolLiteral*>(
      &model.transition_rewards(1)[0].guard());
  ASSERT_NE(nullptr, reward_guard1);
  EXPECT_EQ(true, reward_guard1->value());
  const DoubleLiteral* const reward_expr1 = dynamic_cast<const DoubleLiteral*>(
...
TEST(ParseFileTest, ParsesValidModel) {
  ...
  ASSERT_EQ(1, model.module_commands(0).size());
  EXPECT_EQ(nullptr, model.module_commands(0)[0].action());
  const BoolLiteral* const guard0 = dynamic_cast<const BoolLiteral*>(
      &model.module_commands(0)[0].guard());
  ASSERT_NE(nullptr, guard0);
  EXPECT_EQ(true, guard0->value());
  ASSERT_EQ(1, model.module_commands(0)[0].outcomes().size());
  const DoubleLiteral* const probability0 = dynamic_cast<const DoubleLiteral*>(
      &model.module_commands(0)[0].outcomes()[0].probability());
  ASSERT_NE(nullptr, probability0);
  ...
  EXPECT_EQ("b",
            model.module_commands(0)[0].outcomes()[0].updates()[0].variable());
  const UnaryOperation* const update_expr0 =
      dynamic_cast<const UnaryOperation*>(
          &model.module_commands(0)[0].outcomes()[0].updates()[0].expr());
  ASSERT_NE(nullptr, update_expr0);
  EXPECT_EQ(UnaryOperator::NOT, update_expr0->op());
  const Identifier* const update_expr0_operand =
      dynamic_cast<const Identifier*>(&update_expr0->operand());
  ASSERT_NE(nullptr, update_expr0_operand);
  EXPECT_EQ("b", update_expr0_operand->name());
